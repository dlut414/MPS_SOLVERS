...
///update pos and vel
//////////////////////////////
template<char _dir> void MPS_CPU_IMPL::update_pos_vel()
{
    switch(_dir)
    {
        case 1:
...
//////////////////////////////
template<char _dir> void MPS_CPU_IMPL::CN_buildCG()
{
    int _num;
    int _self;
    int _cell;
    ...
    real _weight;
    real _ONE_OVER_R; // 1 / R, for b
    real3 _DT_BY_G_OVER_R; // dt * g / R, for b
    real3 _dr;

    _ONE_OVER_R = (d_lambda * d_nzero) / (i_dim * d_niu * d_dt);
    _DT_BY_G_OVER_R = (d_lambda * d_nzero) / (i_dim * d_niu) * G;
