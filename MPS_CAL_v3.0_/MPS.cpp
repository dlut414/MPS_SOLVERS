...
MPS::~MPS()
{
    ...
    delete[] d_n; d_n = NULL;
    delete[] d_omp_tmp; d_omp_tmp = NULL;
...
void MPS::CreateGeo()
{
    ...
    d_n = new real[i_np];
#ifdef CPU_OMP
    d_omp_tmp = new real[i_np * OMP_THREADS];
    d3_omp_tmp = new real3[i_np * OMP_THREADS];
#else
    d_phat = new real[i_np];
    d3_omp_tmp = new real3[i_np];
#endif
    ...
    for(int i=0;i<i_np;i++)
    {
        fscanf(_fid, "%lf%lf%lf",&d3_pos[i].x,&d3_pos[i].y,&d3_pos[i].z);
    }
    for(int i=0;i<i_np;i++)
    {
        fscanf(_fid, "%lf%lf%lf",&d3_vel[i].x,&d3_vel[i].y,&d3_vel[i].z);
    }
    for(int i=0;i<i_np;i++)
    {
        fscanf(_fid, "%lf",&d_press[i]);
    }
...
void MPS::CalOnce()
{
    ...
    allocate_cell_list();//calculate num of cells and allocate memory
...
///////////////////////////////////////////////////////
void MPS::allocate_cell_list()
{
    char str[256];
    int _neighbor; //for i_link_cell
    FILE* _fid_log;
    ...
    i_cell_dx = int(d_cell_dx / d_cell_size) + 1;
    i_cell_dy = int(d_cell_dy / d_cell_size) + 1;
    i_cell_dz = int(d_cell_dz / d_cell_size) + 1;
    ...
    i_link_cell = new int*[i_num_cells];
    for(int i=0;i<i_num_cells;i++) i_link_cell[i] = new int[_neighbor + 1];

    i_part_in_cell = new int[i_num_cells];
    i_cell_start = new int[i_num_cells];
    i_cell_end = new int[i_num_cells];
    i_cell_list = new int[i_np];
