...
namespace cudaker
{
    ...
    void dev_cal_n(mytype::real* const d_n,
             ...
             const mytype::integer i_np)
    {
        mytype::integer* dev_i_cell_list;
        mytype::integer* dev_i_link_cell;
        mytype::integer* dev_i_cell_start;
        mytype::integer* dev_i_cell_end;
        mytype::real3*   dev_d3_pos;
        mytype::real*    dev_d_n;

        checkCuda( cudaMalloc(&dev_i_cell_list,  i_np*sizeof(mytype::integer)) );
        checkCuda( cudaMalloc(&dev_i_link_cell,  28*i_num_cells*sizeof(mytype::integer)) );
        checkCuda( cudaMalloc(&dev_i_cell_start, i_num_cells*sizeof(mytype::integer)) );
        checkCuda( cudaMalloc(&dev_i_cell_end,   i_num_cells*sizeof(mytype::integer)) );
        checkCuda( cudaMalloc(&dev_d3_pos,       i_np*sizeof(mytype::real3)) );
        checkCuda( cudaMalloc(&dev_d_n,          i_np*sizeof(mytype::real)) );

        checkCuda( cudaMemcpy(dev_i_cell_list, i_cell_list, i_np*sizeof(mytype::integer), cudaMemcpyHostToDevice) );
        checkCuda( cudaMemcpy(dev_i_link_cell, i_link_cell, 28*i_num_cells*sizeof(mytype::integer), cudaMemcpyHostToDevice) );
        checkCuda( cudaMemcpy(dev_i_cell_start, i_cell_start, i_num_cells*sizeof(mytype::integer), cudaMemcpyHostToDevice) );
        checkCuda( cudaMemcpy(dev_i_cell_end, i_cell_end, i_num_cells*sizeof(mytype::integer), cudaMemcpyHostToDevice) );
        checkCuda( cudaMemcpy(dev_d3_pos, d3_pos, i_np*sizeof(mytype::real3), cudaMemcpyHostToDevice) );
        ...
        checkCuda( cudaMemcpy(d_n, dev_d_n, i_np*sizeof(mytype::real), cudaMemcpyDeviceToHost) );
    ...
    __global__ void kerCalDash_tmp ( mytype::real3* const dev_d3_vel, mytype::real3* const dev_d3_pos, mytype::real3* const dev_d3_tmp,
                               const mytype::real* const dev_d_press, const mytype::integer* const dev_i_type,
                               const mytype::integer* const dev_i_cell_list, const mytype::integer* const dev_i_link_cell,
                               const mytype::integer* const dev_i_cell_start, const mytype::integer* const dev_i_cell_end,
                               ...
                               const mytype::integer i_np )
    {
        ...
        if(i < i_np)
        {
                ...
                if(__cell < i_num_cells)
                {
                    ...
                    for(mytype::integer j=__start;j<__end;j++)
                    {
                        if(j != i)
                        {
                            ...
                            _ret.x = _ret.x + (dev_d_press[j] - _hat_p) / __rr * dev_d_weight(d_rzero, sqrt(__rr)) * __dr.x;
                            _ret.y = _ret.y + (dev_d_press[j] - _hat_p) / __rr * dev_d_weight(d_rzero, sqrt(__rr)) * __dr.y;
                            _ret.z = _ret.z + (dev_d_press[j] - _hat_p) / __rr * dev_d_weight(d_rzero, sqrt(__rr)) * __dr.z;
                        }
                    }
                }
            }

            _ret.x = i_dim * d_one_over_nzero * _ret.x;
            _ret.y = i_dim * d_one_over_nzero * _ret.y;
            _ret.z = i_dim * d_one_over_nzero * _ret.z;
            /*-----------------------------------------*/
            ...
            if(dev_i_type[i] == 0)
            {
                dev_d3_tmp[i].x = - d_dt * d_one_over_rho * _ret.x;
                dev_d3_tmp[i].y = - d_dt * d_one_over_rho * _ret.y;
                dev_d3_tmp[i].z = - d_dt * d_one_over_rho * _ret.z;
            }
            /*--------------------------------*/
        }
    }

    __global__ void kerCalDash( mytype::real3* const dev_d3_vel, mytype::real3* const dev_d3_pos,
                          const mytype::real3* const dev_d3_tmp, const mytype::integer* const dev_i_type,
                          const mytype::real d_dt,
                          const mytype::integer i_np )
    {
        const mytype::integer i = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        if(i < i_np)
        {
            if(dev_i_type[i] == 0)
            {
                dev_d3_vel[i].x = dev_d3_vel[i].x + dev_d3_tmp[i].x;
                dev_d3_vel[i].y = dev_d3_vel[i].y + dev_d3_tmp[i].y;
                dev_d3_vel[i].z = dev_d3_vel[i].z + dev_d3_tmp[i].z;

                dev_d3_pos[i].x = dev_d3_pos[i].x + d_dt * dev_d3_tmp[i].x;
                dev_d3_pos[i].y = dev_d3_pos[i].y + d_dt * dev_d3_tmp[i].y;
                dev_d3_pos[i].z = dev_d3_pos[i].z + d_dt * dev_d3_tmp[i].z;
            }
        }
    }
               ...
               const mytype::integer i_np)
    {
        ...
        mytype::integer* dev_i_cell_start;
        mytype::integer* dev_i_cell_end;
        mytype::integer* dev_i_type;

        mytype::real3*   dev_d3_vel;
        mytype::real3*   dev_d3_pos;
        mytype::real3*   dev_d3_tmp;
        mytype::real*    dev_d_press;
        ...
        checkCuda( cudaMalloc(&dev_i_cell_start, i_num_cells    *   sizeof(mytype::integer)) );
        checkCuda( cudaMalloc(&dev_i_cell_end,   i_num_cells    *   sizeof(mytype::integer)) );
        checkCuda( cudaMalloc(&dev_i_type,       i_np           *   sizeof(mytype::integer)) );

        checkCuda( cudaMalloc(&dev_d3_vel,       i_np   *   sizeof(mytype::real3)) );
        checkCuda( cudaMalloc(&dev_d3_pos,       i_np   *   sizeof(mytype::real3)) );
        checkCuda( cudaMalloc(&dev_d3_tmp,       i_np   *   sizeof(mytype::real3)) );
        checkCuda( cudaMalloc(&dev_d_press,      i_np   *   sizeof(mytype::real)) );
        ...
        checkCuda( cudaMemcpy(dev_i_cell_start, i_cell_start, i_num_cells       *   sizeof(mytype::integer),    cudaMemcpyHostToDevice) );
        checkCuda( cudaMemcpy(dev_i_cell_end,   i_cell_end,   i_num_cells       *   sizeof(mytype::integer),    cudaMemcpyHostToDevice) );
        checkCuda( cudaMemcpy(dev_i_type,       i_type,       i_np              *   sizeof(mytype::integer),    cudaMemcpyHostToDevice) );

        checkCuda( cudaMemcpy(dev_d3_vel,       d3_vel,   i_np  *   sizeof(mytype::real3),  cudaMemcpyHostToDevice) );
        checkCuda( cudaMemcpy(dev_d3_pos,       d3_pos,   i_np  *   sizeof(mytype::real3),  cudaMemcpyHostToDevice) );
        checkCuda( cudaMemcpy(dev_d3_tmp,       d3_tmp,   i_np  *   sizeof(mytype::real3),  cudaMemcpyHostToDevice) );
        checkCuda( cudaMemcpy(dev_d_press,      d_press, i_np   *   sizeof(mytype::real),   cudaMemcpyHostToDevice) );

        ///call routines
        kerCalDash_tmp<<<(i_np+NUM_THREADS-1)/NUM_THREADS, NUM_THREADS>>>(  dev_d3_vel, dev_d3_pos, dev_d3_tmp,
                                                                            dev_d_press, dev_i_type,
                                                                            dev_i_cell_list, dev_i_link_cell,
                                                                            dev_i_cell_start, dev_i_cell_end,
                                                                            d_dt,
                                                                            d_one_over_rho,
                                                                            d_one_over_nzero,
                                                                            d_rzero,
                                                                            ...
                                                                            i_num_cells,
                                                                            i_np );
        kerCalDash<<<(i_np+NUM_THREADS-1)/NUM_THREADS, NUM_THREADS>>>( dev_d3_vel, dev_d3_pos, dev_d3_tmp,
                                                                       dev_i_type,
                                                                       d_dt,
                                                                       i_np );

        ///copy back
        checkCuda( cudaMemcpy(d3_vel, dev_d3_vel, i_np  *   sizeof(mytype::real3),  cudaMemcpyDeviceToHost) );
        checkCuda( cudaMemcpy(d3_pos, dev_d3_pos, i_np  *   sizeof(mytype::real3),  cudaMemcpyDeviceToHost) );
                                     ...
                                     const mytype::integer i_np )
    {
        ...
        if(i < i_np)
        {
            ...
            dev_d_press[i] = _tmp > 0.0 ? _tmp : 0.0; //pressure before i is already changed
        }
    }
                    ...
                    const mytype::integer i_np)
    {
        ...
        ///allocate
        checkCuda( cudaMalloc(&dev_i_type,       i_np*sizeof(mytype::integer)) );
        checkCuda( cudaMalloc(&dev_i_normal,     i_np*sizeof(mytype::integer)) );
        ...
        ///copy in
        checkCuda( cudaMemcpy(dev_i_type,       i_type,       i_np*sizeof(mytype::integer),    cudaMemcpyHostToDevice) );
        checkCuda( cudaMemcpy(dev_i_normal,     i_normal,     i_np*sizeof(mytype::integer),    cudaMemcpyHostToDevice) );

        checkCuda( cudaMemcpy(dev_d_press,      d_press,      i_np*sizeof(mytype::real),              cudaMemcpyHostToDevice) );
        checkCuda( cudaMemcpy(dev_d_n,          d_n,          i_np*sizeof(mytype::real),              cudaMemcpyHostToDevice) );
        ...
        ///copy back
        checkCuda( cudaMemcpy(d_press,      dev_d_press,        i_np*sizeof(mytype::real),      cudaMemcpyDeviceToHost) );
    ...
    __global__ void kerCalVisc_tmp( mytype::real3* const dev_d3_vel, mytype::real3* const dev_d3_pos, mytype::real3* const dev_d3_tmp,
                    ...
                    if(__cell < i_num_cells)
                    {
                        ...
                        for(mytype::integer j=__start;j<__end;j++)
                        {
                            if(j != i)
                            {
                                ...
                                _ret.x = _ret.x + __tmp * __du.x;
                                _ret.y = _ret.y + __tmp * __du.y;
                                _ret.z = _ret.z + __tmp * __du.z;
                            }
                        }
                    }
                }

                dev_d3_tmp[i].x = dev_d3_vel[i].x + d_dt * (d_niu * (d_2bydim_over_nzerobylambda) * _ret.x + G.x);
                dev_d3_tmp[i].y = dev_d3_vel[i].y + d_dt * (d_niu * (d_2bydim_over_nzerobylambda) * _ret.y + G.y);
                dev_d3_tmp[i].z = dev_d3_vel[i].z + d_dt * (d_niu * (d_2bydim_over_nzerobylambda) * _ret.z + G.z);
            }
        }
    }

    __global__ void kerCalVisc( mytype::real3* const dev_d3_vel, mytype::real3* const dev_d3_pos, mytype::real3* const dev_d3_tmp,
                          const mytype::integer* const dev_i_type,
                          const mytype::real d_dt,
                          const mytype::integer i_np )
    {
        ...
        if(i < i_np)
        {
            if(dev_i_type[i] == 0)
            {
                dev_d3_vel[i].x = dev_d3_tmp[i].x;
                dev_d3_vel[i].y = dev_d3_tmp[i].y;
                dev_d3_vel[i].z = dev_d3_tmp[i].z;
                    ...
                    const mytype::integer i_np)
    {
        ...
        ///call routines
        kerCalVisc_tmp<<<(i_np+NUM_THREADS-1)/NUM_THREADS, NUM_THREADS>>>(  dev_d3_vel, dev_d3_pos, dev_d3_tmp,
                                                                            dev_d_press, dev_i_type,
                                                                            dev_i_cell_list, dev_i_link_cell,
                                                                            dev_i_cell_start, dev_i_cell_end,
                                                                            mytype::G,
                                                                            ...
                                                                            i_num_cells,
                                                                            i_np );
        kerCalVisc<<<(i_np+NUM_THREADS-1)/NUM_THREADS, NUM_THREADS>>>( dev_d3_vel, dev_d3_pos, dev_d3_tmp,
                                                                       dev_i_type,
                                                                       d_dt,
                                                                       i_np );
    ...
    __global__ void kerCalCol_tmp( mytype::real3* const dev_d3_vel, mytype::real3* const dev_d3_pos, mytype::real3* const dev_d3_tmp,
                             const mytype::integer* const dev_i_type,
                             const mytype::integer* const dev_i_cell_list,
                             const mytype::integer* const dev_i_link_cell,
                             const mytype::integer* const dev_i_cell_start,
                    ...
                    if(__cell < i_num_cells)
                    {
                        ...
                        for(mytype::integer j=__start;j<__end;j++)
                        {
                            if(j != i)
                            {
                                ...
                                mytype::real __one_over_ds = 1.0 / __ds;
    ...
    __global__ void kerCalCol( mytype::real3* const dev_d3_vel, mytype::real3* const dev_d3_pos, mytype::real3* const dev_d3_tmp,
                         const mytype::real d_dt,
                         const mytype::integer i_np )
    {
        const mytype::integer i = blockIdx.x * blockDim.x + threadIdx.x;
              ...
              const mytype::integer i_np)
    {
        ...
        ///call routines
        kerCalCol_tmp<<<(i_np+NUM_THREADS-1)/NUM_THREADS, NUM_THREADS>>>(  dev_d3_vel, dev_d3_pos, dev_d3_tmp,
                                                                           dev_i_type,
                                                                           dev_i_cell_list, dev_i_link_cell,
                                                                           dev_i_cell_start, dev_i_cell_end,
                                                                           d_dt,
                                                                           ...
                                                                           i_num_cells,
                                                                           i_np );
        kerCalCol<<<(i_np+NUM_THREADS-1)/NUM_THREADS, NUM_THREADS>>>( dev_d3_vel, dev_d3_pos, dev_d3_tmp,
                                                                      d_dt,
                                                                      i_np );
