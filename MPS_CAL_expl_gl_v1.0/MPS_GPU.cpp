...
void MPS_GPU::Initial()
{
    ...
    d3_tmp1 = new real3[i_np];
    d3_tmp2 = new real3[i_np];
...
void MPS_GPU::devInit()
{
    ...
    checkCuda( cudaMalloc(&dev_d3_tmp1,            i_np*sizeof(real3)) );
    checkCuda( cudaMalloc(&dev_d3_tmp2,            i_np*sizeof(real3)) );
...
void MPS_GPU::devFree()
{
    ...
    checkCuda( cudaFree(dev_d3_tmp1) );
    checkCuda( cudaFree(dev_d3_tmp2) );
...
void MPS_GPU::commHostToDevice()
{
    ...
    checkCuda( cudaMemcpy(dev_d3_pos,       d3_pos,         i_np*sizeof(real3),             cudaMemcpyHostToDevice) );
    checkCuda( cudaMemcpy(dev_d3_vel,       d3_vel,         i_np*sizeof(real3),             cudaMemcpyHostToDevice) );
    checkCuda( cudaMemcpy(dev_d3_tmp2,       d3_pos,         i_np*sizeof(real3),             cudaMemcpyHostToDevice) );
    checkCuda( cudaMemcpy(dev_d3_tmp1,       d3_vel,         i_np*sizeof(real3),             cudaMemcpyHostToDevice) );
}
...
{
    checkCuda( cudaMemcpy(d_n,          dev_d_n,            i_np*sizeof(real),              cudaMemcpyDeviceToHost) );
    checkCuda( cudaMemcpy(d_press,      dev_d_press,        i_np*sizeof(real),              cudaMemcpyDeviceToHost) );
    checkCuda( cudaMemcpy(d3_pos,       dev_d3_pos,         i_np*sizeof(real3),             cudaMemcpyDeviceToHost) );
    checkCuda( cudaMemcpy(d3_vel,       dev_d3_vel,         i_np*sizeof(real3),             cudaMemcpyDeviceToHost) );
}
...
void MPS_GPU::calDash()
{
    ...
    cudaker::dev_calDash( dev_d3_tmp1,
                          dev_d3_tmp2,
                          dev_d3_vel,
                          dev_d3_pos,
                          dev_d_press,
    ...
    for(integer i = 0; i < i_np; i++)
    {
        ...
        if(i_type[i] == 0)
        {
            ...
            d3_tmp1[i] = d3_tmp1[i] + __tmp;
            d3_tmp2[i] = d3_tmp2[i] + d_dt * __tmp;
        }
    }
...
void MPS_GPU::collision()
{
    ...
    cudaker::dev_calCol( dev_d3_tmp1,
                         dev_d3_tmp2,
                         dev_d3_vel,
                         dev_d3_pos,
                         dev_i_type,
    ...
    for(integer i=0;i<i_np;i++)
    {
        ...
        d3_tmp1[i] = d3_tmp1[i] + _crt;
        d3_tmp2[i] = d3_tmp2[i] + d_dt * _crt;
    }
...
void MPS_GPU::calVisc_expl()
{
    ...
    cudaker::dev_calVisc_expl( dev_d3_tmp1,
                               dev_d3_tmp2,
                               dev_d3_vel,
                               dev_d3_pos,
                               dev_d_press,
    ...
    for(integer i = 0; i < i_np; i++)
    {
        ...
        if(i_type[i] == 0)
        {
            d3_tmp1[i] = d3_vel[i] + d_dt * (d_niu * d3_LapVel(i) + G);
            d3_tmp2[i] = d3_pos[i] + d_dt * d3_tmp1[i];
        }
    }
...
void MPS_GPU::updatePV()
{
#ifdef GPU_CUDA

    cudaker::dev_updatePV( dev_d3_vel,
                           dev_d3_pos,
                           dev_d3_tmp1,
                           dev_d3_tmp2,
                           i_np );
