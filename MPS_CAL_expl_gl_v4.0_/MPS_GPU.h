...
private:
    ...
    void sort_all_host();       //sort part by cell
    void sort_all_dev();        //sort part by cell
...
private:
    /*----------------memory on device----------------*/
    matrix_COO<integer, real>* dev_matrixA;
    real*    dev_r_r0;
    real*    dev_r_r;
    real*    dev_r_p;
    real*    dev_r_s;
...
void WriteCase()
{
    FILE* _out = fopen("./out/out.out" , "wt");

    for(integer i = 0; i < i_np; i++)
    {
        fprintf(_out , "%d %d %f %f %f %f %f %f %f " ,
                i_id[i], i_type[i],
