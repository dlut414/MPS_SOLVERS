...
MPS::MPS()
{
    ...
    i_cell_end = NULL;
    //i_normal = NULL;
    matrixA = new matrix_COO<integer, real>;
}
...
void MPS::allocCell()
{
    ...
    geo.d_cell_size     = (d_rzero > d_rlap ? d_rzero : d_rlap);

    geo.d_cell_left     = geo.d_cell_right = d3_pos[0].x;
    geo.d_cell_back     = geo.d_cell_front = d3_pos[0].y;
    geo.d_cell_bottom   = geo.d_cell_top   = d3_pos[0].z;

    for(integer i=0;i<i_np;i++)
    {
        if(d3_pos[i].x < geo.d_cell_left)   geo.d_cell_left     = d3_pos[i].x;
        if(d3_pos[i].x > geo.d_cell_right)  geo.d_cell_right    = d3_pos[i].x;

        if(d3_pos[i].y < geo.d_cell_back)   geo.d_cell_back     = d3_pos[i].y;
        if(d3_pos[i].y > geo.d_cell_front)  geo.d_cell_front    = d3_pos[i].y;

        if(d3_pos[i].z < geo.d_cell_bottom) geo.d_cell_bottom   = d3_pos[i].z;
        if(d3_pos[i].z > geo.d_cell_top)    geo.d_cell_top      = d3_pos[i].z;
    }

    geo.d_cell_left     -= d_max_dis;
    geo.d_cell_right    += d_max_dis;
    geo.d_cell_back     -= d_max_dis;
    geo.d_cell_front    += d_max_dis;
    geo.d_cell_bottom   -= d_max_dis;
    geo.d_cell_top      += d_max_dis;

    if(i_dim == 2)
    {
        geo.d_cell_back = geo.d_cell_front = 0.0;
    }

    geo.d_cell_dx = geo.d_cell_right - geo.d_cell_left;
    geo.d_cell_dy = geo.d_cell_front - geo.d_cell_back;
    geo.d_cell_dz = geo.d_cell_top   - geo.d_cell_bottom;

    geo.i_cell_dx = integer(geo.d_cell_dx / geo.d_cell_size) + 1;
    geo.i_cell_dy = integer(geo.d_cell_dy / geo.d_cell_size) + 1;
    geo.i_cell_dz = integer(geo.d_cell_dz / geo.d_cell_size) + 1;

    if(geo.i_cell_dx <=2 || geo.i_cell_dz <=2) //if too thin, error in cell list
    {
        printf("cells are too thin ! -> dx: %d, dz: %d\n", geo.i_cell_dx, geo.i_cell_dz);
        exit(5);
    }
    if(geo.i_cell_dy <=2 && i_dim == 3) //if too thin, error in cell list
    {
        printf("cells are too thin ! -> dy: %d\n", geo.i_cell_dy);
        exit(5);
    }

    geo.i_cell_sheet    = geo.i_cell_dx     * geo.i_cell_dy;
    geo.i_num_cells     = geo.i_cell_sheet  * geo.i_cell_dz;

    i_link_cell     = new integer[28 * geo.i_num_cells];
    i_part_in_cell  = new integer[geo.i_num_cells];
    i_cell_start    = new integer[geo.i_num_cells+1];
    i_cell_end      = new integer[geo.i_num_cells+1];
    i_cell_list     = new integer[i_np];

    memAdd(sizeof(integer), 28 * geo.i_num_cells);
    memAdd(sizeof(integer),      geo.i_num_cells);
    memAdd(sizeof(integer),      geo.i_num_cells);
    memAdd(sizeof(integer),      geo.i_num_cells);
    memAdd(sizeof(integer),      i_np);

    _fid_log = fopen(LOG_NAME, "at");
    sprintf(_str, "successfully Allocated cell list!\n");
    throwScn(_str);
...
/////////////////////////////////////////////////////////////////////
///alloc mem for matrix A
/////////////////////////////////////////////////////////////////////
void MPS::allocMatrix()
{
    real _d_size = d_rzero > d_rlap ? d_rzero : d_rlap;
    integer _sizeMax = 50 * integer((_d_size/d_dp) * (_d_size/d_dp) * (_d_size/d_dp)) * i_np + 1;

    matrixA->i_sizeA = _sizeMax;
    matrixA->i_sizeB = i_np;

    //matrixA->r_ptrAij = new real[_sizeMax];
    /*
    matrixA->reset();

    memAdd(sizeof(integer),   _sizeMax);
    memAdd(sizeof(real),      _sizeMax);

    char _str[256];
    FILE* _fid_log;
    _fid_log = fopen(LOG_NAME, "at");
    sprintf(_str, "successfully Allocated matrix A!\n");
    throwScn(_str);
    throwLog(_fid_log, _str);
    fclose(_fid_log);
    */
